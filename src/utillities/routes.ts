...
routes.get('/api/full?', (req, res) => {
    // resolve path
    const fullImage = path.resolve(__dirname, `../../images/full/${req.query.filename}.jpg`);

    // Reading the file
    fs.readFile(fullImage,
...
routes.get('/api/resize?', async (req, res) => {
    // resolve path
    const fullImagePath = path.resolve(__dirname, `../../images/full/${req.query.filename}.jpg`);
    const resizedImagePath = path.resolve(__dirname, `../../images/thumb/${req.query.filename}-${req.query.width}x${req.query.height}.jpg`);
    ...
    if (fs.existsSync(resizedImagePath)) {
        res.redirect(`/api/resized?filename=${req.query.filename}&width=${req.query.width}&height=${req.query.height}`);
    } else {
...
routes.get('/api/resized?', (req, res) => {
    ...
    if (fs.existsSync(resizedImagePath)) {
        ...
        res.redirect(`/api/resize?filename=${req.query.filename}&width=${req.query.width}&height=${req.query.height}`);
    }
});
